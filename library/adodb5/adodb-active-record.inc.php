...
class ADODB_Active_Record {
	...
	// false on error
  // Modified Save() to Save($IDKey=null) for fix mssql insert IDENTITY_INSERT value
	function Save($IDKey=null)
	{
		if ($this->_saved) $ok = $this->Update();
		else $ok = $this->Insert($IDKey);
	...
	// false on error
  // Modified Insert() to Insert($IDKey=null) for fix mssql insert IDENTITY_INSERT value
	function Insert($IDKey=null)
	{
		$db = $this->DB(); if (!$db) return false;
		$cnt = 0;
		$table = $this->TableInfo();
        ...
        if ($name==$IDKey) continue; // Modified for fix mssql insert IDENTITY_INSERT value
				$valarr[] = $val;
